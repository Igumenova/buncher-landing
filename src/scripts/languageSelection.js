...
export const setLanguageSelectorBehavior = function () {
    ...
    langContainers.forEach(el => langContainersMap[el.dataset.key][el.dataset.lang] = el);
    ...
    const chooseOption = (curOption) => {
        changeLanguage(curOption.dataset.lang, langElements, langContainers, phoneImages, langLinks);
        hideDropdown();
    ...
    const moveSelection = (offset) => {
        selectedIndex = Math.max(0, Math.min(optionElements.length - 1, selectedIndex + offset));
        updateSelection();
    }
